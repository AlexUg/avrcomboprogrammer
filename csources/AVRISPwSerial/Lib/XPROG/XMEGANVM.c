 ...
 */
static void
XMEGANVM_SendAddress (const uint32_t AbsoluteAddress)
{
 ...
 */
static void
XMEGANVM_SendNVMRegAddress (const uint8_t Register)
{
 ...
 */
bool
XMEGANVM_WaitWhileNVMBusBusy (void)
{
 ...
 */
bool
XMEGANVM_WaitWhileNVMControllerBusy (void)
{
  ...
  for (;;)
    {
      ...
      XPROGTarget_SendByte (
          PDI_CMD_LD(PDI_POINTER_INDIRECT, PDI_DATASIZE_1BYTE));
...
XMEGANVM_EnablePDI (void)
{
  ...
  for (uint8_t i = sizeof(PDI_NVMENABLE_KEY
  ); i > 0; i--)
    XPROGTarget_SendByte (PDI_NVMENABLE_KEY
    [i - 1]);
 ...
 */
bool
XMEGANVM_GetMemoryCRC (const uint8_t CRCCommand, uint32_t* const CRCDest)
{
  *CRCDest = 0;
  ...
  XPROGTarget_SendByte (
      PDI_CMD_LD(PDI_POINTER_INDIRECT_PI, PDI_DATASIZE_1BYTE));
 ...
 */
bool
XMEGANVM_ReadMemory (const uint32_t ReadAddress, uint8_t* ReadBuffer,
                     uint16_t ReadSize)
{
  /* Wait until the NVM controller is no longer busy */
  if (!(XMEGANVM_WaitWhileNVMControllerBusy ()))
  ...
  if (ReadSize > 1)
    {
      ...
      XPROGTarget_SendByte (
          PDI_CMD_ST(PDI_POINTER_DIRECT, PDI_DATASIZE_4BYTES));
  ...
  else
    {
      ...
      XPROGTarget_SendByte (
          PDI_CMD_LDS(PDI_DATASIZE_4BYTES, PDI_DATASIZE_1BYTE));
      XMEGANVM_SendAddress (ReadAddress);
      *(ReadBuffer++) = XPROGTarget_ReceiveByte ();
    }
 ...
 */
bool
XMEGANVM_WriteByteMemory (const uint8_t WriteCommand,
                          const uint32_t WriteAddress, const uint8_t Byte)
{
  /* Wait until the NVM controller is no longer busy */
  if (!(XMEGANVM_WaitWhileNVMControllerBusy ()))
    return false;
 ...
 */
bool
XMEGANVM_WritePageMemory (const uint8_t WriteBuffCommand,
                          const uint8_t EraseBuffCommand,
                          const uint8_t WritePageCommand,
                          const uint8_t PageMode, const uint32_t WriteAddress,
  ...
  if (PageMode & XPROG_PAGEMODE_ERASE)
    {
      ...
      XPROGTarget_SendByte (
          PDI_CMD_STS(PDI_DATASIZE_4BYTES, PDI_DATASIZE_1BYTE));
      XMEGANVM_SendNVMRegAddress (XMEGA_NVM_REG_CMD);
  ...
  if (WriteSize)
    {
      ...
      XPROGTarget_SendByte (
          PDI_CMD_ST(PDI_POINTER_INDIRECT_PI, PDI_DATASIZE_1BYTE));
      while (WriteSize--)
