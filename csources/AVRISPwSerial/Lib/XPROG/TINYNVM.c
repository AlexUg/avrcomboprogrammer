...
static void
TINYNVM_SendPointerAddress (const uint16_t AbsoluteAddress)
{
 ...
 */
static void
TINYNVM_SendReadNVMRegister (const uint8_t Address)
{
 ...
 */
static void
TINYNVM_SendWriteNVMRegister (const uint8_t Address)
{
  /* The TPI command for reading from the I/O space uses strange addressing, where the I/O address's upper
   * two bits of the 6-bit address are shifted left once - use function to reduce code size */
 ...
 */
bool
TINYNVM_WaitWhileNVMBusBusy (void)
{
 ...
 */
bool
TINYNVM_WaitWhileNVMControllerBusy (void)
{
  /* Poll the STATUS register to check to see if NVM access has been enabled */
  for (;;)
    {
...
TINYNVM_EnableTPI (void)
{
  ...
  for (uint8_t i = sizeof(TPI_NVMENABLE_KEY
  ); i > 0; i--)
    XPROGTarget_SendByte (TPI_NVMENABLE_KEY
    [i - 1]);
 ...
 */
bool
TINYNVM_ReadMemory (const uint16_t ReadAddress, uint8_t* ReadBuffer,
                    uint16_t ReadSize)
{
 ...
 */
bool
TINYNVM_WriteMemory (const uint16_t WriteAddress, uint8_t* WriteBuffer,
 ...
 */
bool
TINYNVM_EraseMemory (const uint8_t EraseCommand, const uint16_t Address)
{
  /* Wait until the NVM controller is no longer busy */
  if (!(TINYNVM_WaitWhileNVMControllerBusy ()))
    return false;
